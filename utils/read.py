...
def spiral_tramsform(transform_fp, template_fp, ds_factors, seq_length, dilation):
    ...
    spiral_indices_list = [
        mesh_sampling.preprocess_spiral(tmp['face'][idx], seq_length[idx], tmp['vertices'][idx], dilation[idx])#.to(device)
        for idx in range(len(tmp['face']) - 1)
    ]

    down_transform_list = [
        mesh_sampling.to_sparse(down_transform)#.to(device)
        for down_transform in tmp['down_transform']
    ]
    up_transform_list = [
        mesh_sampling.to_sparse(up_transform)#.to(device)
